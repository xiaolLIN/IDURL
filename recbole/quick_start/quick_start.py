...
def run_recbole(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True):
    ...
    # dataset splitting
    train_data, valid_data, test_data, split_test_data_loaders, split_test_statis = data_preparation(config, dataset)
    ...
    return_info = {
        ...
        'model': ((trainer.saved_model_file).split('.')[0]).split('/')[-1],
        'best_valid_result': best_valid_result,
        'test_result': test_result,
        'split_test_statis': split_test_statis
    }

    if config['split_eval']:
        for degree, loaders in enumerate(split_test_data_loaders):
            if loaders is not None:
                test_split_result = trainer.evaluate(loaders, load_best_model=saved, show_progress=config['show_progress'])
                logger.info('drift degree : {}'.format(str(degree)))
                logger.info(set_color('drift degree-{} res'.format(str(degree)), 'yellow') + f': {test_split_result}')
                return_info['drift degree-{}_res'.format(str(degree))] = test_split_result
            else:
                return_info['drift degree-{}_res'.format(str(degree))] = None
