...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        if 'IDURL' in config['model']:
            saved_model_file = '{}-K{}-IDRA{}-lmd-{}-IDRD_lmd{}-H{}-L{}-{}.pth'.format(config['model'],
                                                                                     str(config['n_newc_repr']),
                                                                                     str(config['idra']),
                                                                                     str(config['align_lambda']).replace('.', '_'),
                                                                                     str(config['disen_lambda']).replace('.', '_'),
                                                                                     str(config['n_heads']),
                                                                                     str(config['n_layers']),
                                                                                     get_local_time())
        else:
        ...
        ensure_dir(self.checkpoint_dir + '/' + config.dataset_name)
        self.saved_model_file = os.path.join(self.checkpoint_dir + '/' + config.dataset_name, saved_model_file)
