...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        # |V| x topn, attain the topn pop items of the hierarchical category w.r.t each item in the input seq from valid, test data.
        # note that each item itself can be one of the topn items.
        self.cate_topn_items = None
        self.hier_cate_map2items = None   # dict key: hier cate str, value: item list []
        self.item_cate_info = None
    ...
    def _data_filtering(self):
        ...
        self._filter_inter_by_user_or_item()    # 移除 不在 user（item）set 出现过的 u（i）的 交互记录
    ...
    def build(self):
        ...
        # divide the user set into groups according to drift degree
        split_test_sets = []
        split_test_statis = { }

        if self.config['split_eval'] == 1:
            test_data = datasets[-1]
            item_cate_info = test_data.get_item_feature()['categories']
            test_inter_data = test_data.inter_feat
            test_inter_length = test_inter_data['item_length'].numpy()
            user_ids = test_inter_data['user_id'].numpy()  #   user
            grd_truths = test_inter_data['business_id'] if self.config['dataset'] == 'yelp' else test_inter_data['item_id']
            item_input_seqs = test_inter_data['business_id_list'] if self.config['dataset'] == 'yelp' else test_inter_data['item_id_list']
            seq_len = item_input_seqs.size(-1)

            split_eval_inds_dict = { }  # split_eval_inds_dict[newc_degree]
            for i in range(self.config['n_newc_repr'] + 1):
                split_eval_inds_dict[i] = [ ]

            for i, uid in enumerate(user_ids):
                newc_degree = test_inter_data['newc_degree'][i]
                split_eval_inds_dict[newc_degree.item()].append(i)

            for degree, inds_list in split_eval_inds_dict.items():
                split_test_statis['new_degree-{}'.format(str(degree))] = len(inds_list)
                if len(inds_list) > 0:
                    split_test_sets.append(test_data.copy(test_inter_data[inds_list]))
                else:
                    split_test_sets.append(None)

        datasets = datasets + split_test_sets

        return datasets, split_test_statis
