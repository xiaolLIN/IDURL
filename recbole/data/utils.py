...
def data_preparation(config, dataset, save=False):
    ...
    model_type = config['MODEL_TYPE']
    built_datasets, split_test_statis = dataset.build()
    logger = getLogger()

    train_dataset, valid_dataset, test_dataset = built_datasets[0], built_datasets[1], built_datasets[2]
    train_sampler, valid_sampler, test_sampler = create_samplers(config, dataset,
    ...
    split_test_sets = built_datasets[3:]
    split_test_data_loaders = []
    if config['split_eval'] == 1:
        for split_set in split_test_sets:
            if split_set is not None:
                split_test_data_loaders.append(get_dataloader(config, 'evaluation')(config, split_set, test_sampler, shuffle=False))
            else:
                split_test_data_loaders.append(None)
    ...
    return train_data, valid_data, test_data, split_test_data_loaders, split_test_statis
